...
| `avsubs` | Standalone「AV 翻译」— walk a library folder (recursive by default) and give every MP4 without a same-named `.srt` a `<name>.srt` (zh, global LLM API) next to it, via a `<name>.ja.srt` transcript (WhisperJAV) that is deleted once the `.srt` is written (kept as the resume point when translation does not finish); takes turns on the GPU with Jasna per file (in-process GPU lease) |
