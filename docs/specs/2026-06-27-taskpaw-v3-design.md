...
| 打包 | `cargo tauri build`（壳）+ PyInstaller（`taskpaw-hub` 入口） | **需自建 `scripts/build.py`**。MDCx 的 `build.py` 实为 PyInstaller 打包 `main.py --onefile`，并非"Tauri+后端拆分"，只能参考其 PyInstaller 流程，不能照抄（Kimi P2#5） |
...
> **事实修正（Codex P1#2 + Kimi P1#1/P2#6，均经文件实证）**：
> - `runtime/health_alert_state.json` 是 `{alert_type: timestamp}` **冷却 map**（实测 `{"anthropic_api": 1781322736.38926}`），**不是**事件日志 → 降级为旁路信号。
> - `health_monitor.py` 的告警分类（`OPEND_DOWN/OPEND_KICKED/TRADE_AUTH/BRAIN_DAILY_LOSS_HALT`）**只发邮件**，不输出可消费文件 → agent **无法**"消费这些分类"。但 health_monitor **会 append 到 `runtime/freeze_incidents.jsonl`**（实证 health_monitor.py:239）→ 这才是可消费的告警事件源。
> - 真正可消费的健康/交易信号源：`orchestrator_log.jsonl`（实测含 `runtime_health`/`strategy_error`/`entry_blocked_health`/`startup_health_check` 事件）、`freeze_incidents.jsonl`、`confirmed_fill_ledger.jsonl`、`brain_guard_state.json`。
>
> **⚠️ 文件权限（Codex/Kimi 均漏、本文新增）**：实测 `brain_guard_state.json`、`health_alert_state.json` 为 `-rw-------`（仅 owner），`confirmed_fill_ledger.jsonl` 为 `-rw-r--r--`。**agent 进程必须与 MQT 同用户运行（或显式授读权限）**，否则读不到熔断/冷却状态。#0 勘测必须确认 agent 运行用户对各 `runtime/*` 文件的可读性。
...
| 项 | type_id | 配置 | 告警 |
|---|---|---|---|
| 编排器进程 | `process` | pm2 进程名（**可配**，默认 `orchestrator`，#0 勘测确认 `ecosystem.config.js` 实际名）的 status（区分 online/errored/crash-loop/stopped） | 非 online |
| 编排器心跳 | `heartbeat` | `runtime/orchestrator_heartbeat.json`，按 `next_check_due_utc` **+ watchdog grace** 判定 | 超 due+grace 未更新 = HUNG |
| 冻结事件 | `tail_jsonl` | `runtime/freeze_incidents.jsonl`（watchdog/health_monitor 真实事件日志） | 新增行→告警 |

**第二层 · 交易可用性**
| 项 | type_id | 配置 | 告警/说明 |
|---|---|---|---|
| OpenD 端口 | `tcp_check` | `127.0.0.1:11111` | 非 LISTEN=交易瘫痪。**LISTEN ≠ 已登录/已解锁/订阅可用**，仅粗判 |
| OpenD/健康深度异常 | `tail_jsonl` | `orchestrator_log.jsonl` 的 `strategy_error` / `entry_blocked_health` / `startup_health_check` 事件（实证存在） | **不**消费 health_monitor 邮件分类（不落文件）；以日志事件为准 |
| 上下文泄漏 | `tail_jsonl` | `orchestrator_log.jsonl` 中 `event=="runtime_health"` 行（jsonl 事件，非独立文件），算 `ctx_created-ctx_closed` | **连续 3 个 cycle** 均 >2（阈值可配，Kimi P3#12） |

**第三层 · 交易事件**
| 项 | type_id | 配置 | 说明 |
|---|---|---|---|
| 确认成交 | `tail_jsonl` | `runtime/confirmed_fill_ledger.jsonl`（**真实成交**） | `PLACED` 在 `auto_decisions.jsonl` 的 `executions[]` 内，仅"已提交/接受"，非确认成交 |
| 下单/跳过/止损 intent | `tail_jsonl` | `auto_decisions.jsonl`，区分 `PLACED` / `SKIPPED_PREFLIGHT_*` / 止损 intent | 避免把"提交"当"成交" |
| 每日亏损熔断 | `state_file` | `runtime/brain_guard_state.json` 的 `halted==true && halt_reason=="daily_loss"`（实证字段）；旁路确认 `auto_decisions.jsonl` 出现 `SKIPPED_PREFLIGHT_DAILY_LOSS_HALT` | 触发→高优告警（注意此文件 `rw-------`，见权限说明） |
...
- 本地配置文件启动：`/etc/taskpaw/agent.yaml`（*nix）或 `%APPDATA%/TaskPaw/agent.yaml`（Win）。
...
- **#3 插件 supervisor + schema 子集**：`MonitorPlugin/MonitorInstance` + Supervisor 契约（§4.1：崩溃重启/超时/退避/DEGRADED）+ pydantic/json_schema/ui_schema + 资源上限（§4.4）。先迁 `process/heartbeat/tail_jsonl/tcp_check` 四种打通。
...
| agent 读不到 MQT 的 `rw-------` 状态文件（权限） | 中 | agent 与 MQT 同用户运行或显式授读；#0 勘测逐文件确认可读性 |
...
1. **moomoo 信号清单 + 权限实勘**：确认交易机上各 `runtime/*` 健康/交易信号文件的实际字段、路径、以及 agent 运行用户的可读性。
