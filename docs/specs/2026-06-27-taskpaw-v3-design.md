...
- 因此 **所有 agent 对 Hub 都是远程** → 统一走远程鉴权（Tailscale/VPN + Bearer）或 agent push（§3.2/§9 决策点 2）。**无 loopback/同机情形**。
...
按 §3.2 统一远程方案：agent push 到 Hub webhook，或 Tailscale/VPN + Bearer（§9 决策点 2 与所有远程 agent 一并定）。无 loopback 情形。
...
2. **所有远程 agent 的传输面**（agent 均与 Hub 异机）→ **倾向：agent push 到 Hub webhook**（agent 不监听公网，评审 P1#3）。备选 Tailscale/VPN+Bearer。待确认。
...
| 风险 | 等级 | 缓解 |
|---|---|---|
| 在生产交易机跑额外 agent 影响交易进程 | 高 | agent 只读、低频、资源上限、最小产物、push 模式不对外监听；先在 distill 副本验证 |
