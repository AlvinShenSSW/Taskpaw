...
§9 决策已全定 → 据 §10 建 issue（#0–#6）→ `/afk` 或 `/afk codex` 逐个自治实现 + 双评审。#0 仅需补 moomoo 一台机的事实勘测（附录 B）。
...
## 附录 B · #0 勘测待确认（事实项，§9 决策已全定）

§9 九项决策已全部敲定。剩余仅 moomoo 一台机器的**事实勘测**（实现 #4 前确认）：
1. 进程管理器类型（本机已确认 pm2）与作业名（`ecosystem.config.js`/launchd）。
2. `orchestrator_heartbeat.json` 路径与 grace 阈值。
3. OpenD 端口（默认 11111，可能被 `.env` 改）。
4. pm2 daemon 的存活探测方式（`pm2 ping` / God Daemon 进程）。

（交易层信号不在范围；supervisor 接口与资源默认值见 §4，已在文档定义，实现 #3 时落地。）
