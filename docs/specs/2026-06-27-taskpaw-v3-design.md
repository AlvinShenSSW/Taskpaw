...
5. **事件投递有理论丢失窗口**：`/events` 是"取走即清空"，Hub 取走后落库前崩溃可丢事件。**实战数月未发生**；operator 定为"只优化不重写"（§6 可选 clear-on-ack 闭合此窗口）。
6. **已知遗留 bug**：`taskpaw_hub.py` 写 status 文件处 `os` 未导入；`macsubs.py:35` OpenRouter key 是占位符。V3 一并消除。
7. **退出不干净（恼人）**：Windows app 点窗口 X 是 `withdraw()` 缩进托盘而非退出（`taskpaw.py:1880,2839-2848`），用户以为关了、进程仍在后台，要手动 kill。残留进程占着端口 5678 + 单实例 mutex（`taskpaw.py:2911-2916`）→ 重启实例拿不到端口、Hub 看到假 Stopped。**根因是 UI 与监控塞在同一进程 + 点 X 缩进托盘**；V3 把监控拆成常驻 OS 服务、UI 做成客户端根治——**UI 功能照样全保留**（见 §7.1）。
...
4. **通信稳则不动**：agent↔Hub 通信稳定数月，**保留 poll+/events+单调id+Hub去重**，仅做向后兼容的小优化（可选 clear-on-ack、Hub 转发重试、附加字段，见 §6）。不引入 push、不重写协议。
...
10. **监控后端与 UI 分离 → 干净生命周期**：监控跑在**常驻 OS 服务**（干净 start/stop，stop 终止所有监控线程 + 托管子进程如 lada-cli）；UI 是**连本地服务的客户端**，关窗口只关 UI、监控照常跑。**根治 V2"点 X 缩进托盘、进程残留要手动 kill"**，同时 **UI 功能全保留**（见 §1.3#7、§7.1）。
...
- 因此 **所有 agent 对 Hub 都是远程** → 沿用现有 Hub 轮询 + Bearer，叠加 Tailscale/VPN 私网（§3.2）。**无 loopback/同机情形，不引入 push。**
...
| 打包 | **后端服务**（agent + hub，跨平台）：PyInstaller，注册为 OS 服务。**UI 客户端**（agent 控制台 + hub 仪表盘，共享前端）：`cargo tauri build`（macOS + Windows）。moomoo 只装服务、不装 UI | **需自建 `scripts/build.py`**。MDCx 的 `build.py` 实为 PyInstaller 打包 `main.py --onefile`，只能参考流程不能照抄（Kimi P2#5） |
...
在"保留 poll"前提下做安全优化：
- **agent 对 Hub/网络只暴露 `/ping /status /events`（读为主）**，bind 私网网卡、不 bind `0.0.0.0`。本机**控制 API**（启停 Lada、改配置）走**独立 loopback 端口**（`control_addr`），由本机 UI 客户端访问，**不对网络/Hub 暴露**。
- 传输面：**Tailscale/WireGuard 私网 + per-agent Bearer**（与现有 Bearer 一致，仅收紧绑定与网络层）。
- 沿用 V2 Bearer 鉴权与 401 不清队列语义。
- 桌面壳的"loopback 信任"仅用于 Hub 本机 UI；CORS 只对 Hub UI 放开，agent 不开放 CORS。
- secret/token 经配置文件/env，不走 argv、不进日志。
...
按 §3.2：**保留现有 Hub 轮询模型**，agent 在私网（Tailscale/VPN）暴露 `/ping /status /events` + Bearer，Hub 跨机轮询——与现存 Windows agent 完全一致。无 push、无 loopback。
...
1. **clear-on-ack（可选，最高价值）**：agent 由"取走即清空"改为"Hub 确认收到（下次轮询带上已收 max-id）后再裁剪队列"。同一 `/events` 端点，仅改裁剪时机，闭合"Hub 取走后、落库前崩溃"的丢失窗口。**不改协议形状**，作为优化项，可不做。
...
TaskPaw V2（`machine`）与 MacSubs（`stage/current_file`）字段命名不同，但 Hub 已稳定兼容两者数月——**保持现状**，不强制统一 schema。新增的 `level/title/data` 为可选附加（§6.2），不影响旧字段。
...
## 7. 仓库结构（monorepo；后端服务 + UI 客户端分离）

**核心原则（修复 V2 退出残留 + 保留 agent 全部本地 UI 功能）**：
监控逻辑跑在**常驻后端服务**；UI 是**连到本地服务的客户端**。两者**不同进程**——关 UI 窗口只关 UI，监控服务照常跑；要停监控就停服务（OS 服务管理器，干净）。

```
taskpaw-v3/                  # monorepo
├── core/                    # 共享：events/config/auth/protocol/schema
├── monitors/                # 插件 + registry + supervisor
├── agent/                   # 服务①: taskpaw-agent 后端 —— 监控 + 本机控制 API + /events
│   └── server/{app.py,launcher.py,service.py}   # service.py = Windows Service/launchd 入口
├── hub/                     # 服务②: taskpaw-hub 后端 —— 聚合/SQLite/OpenClaw + Hub API
...
├── scripts/build.py         # 打包：后端服务(agent/hub) + UI 客户端(Tauri)
└── docs/specs/
```

- **agent 保留全部本地操作 UI**（启停 Lada、设监听目录、ComfyUI 配置等，过去数月稳定，**不删**），通过 UI 客户端调本地服务的控制 API。
- **agent 不含** Hub 的跨机聚合 / OpenClaw token / SQLite 仪表盘。
- **moomoo 例外**：只装 agent 后端服务、**不装 UI**（读为主的四体征，无需本机控制台），暴露面最小。
...
- 字段：`server_id`、`bind_addr`（私网网卡，对 Hub 暴露 `/ping /status /events`）、`control_addr`（loopback，对本机 UI 暴露控制 API）、`api_token`、`monitors[]`。
- 配置也可由本机 UI 客户端改写（→ 写回配置 + 热生效），或运维脚本下发。

### 7.1 干净生命周期（根治 V2 退出残留）

V2 把 **UI 与监控塞在同一进程** + 点 X 缩进托盘 → 进程残留要手动 kill（§1.3#7）。V3 拆开后：

- **监控后端 = 常驻 OS 服务**：Windows 注册 **Windows Service**（pywin32）或 NSSM 包装（`sc stop` 一键停）；macOS **launchd** plist（与 moomoo/MacSubs 现状一致）。
- **优雅停机**：服务 stop / SIGTERM → supervisor 依次 `stop()` 所有 MonitorInstance（停轮询线程）+ **terminate 托管子进程（lada-cli managed 模式）** + 释放端口 → 退出。无残留、无僵尸。
- **UI 客户端 = 普通窗口**：关窗口只关 UI，后端服务不受影响（监控继续）；**不再有托盘陷阱**。UI 里提供显式"启停某监控 / 停止本机服务"按钮，调控制 API。
- **单实例**：保留命名 mutex/端口检查；因停机干净，不再"残留占端口→新实例拿不到→Hub 假 Stopped"（§1.3#7）。
- **Hub 同理**：Hub 后端也是常驻服务；Hub 的 Tauri 仪表盘是客户端，**关窗口不停聚合**。（若 Hub 采用 MDCx 式"壳 spawn 后端"，则壳退出杀子进程；若后端独立常驻，则关壳不影响后端——二选一，§9 定。）
...
2. **所有远程 agent 的传输面**（agent 均与 Hub 异机）→ **已定：保留现有 Hub 轮询 + Bearer，叠加 Tailscale/VPN 私网 + 收紧绑定**（operator："通信稳定数月不大改"）。不引入 push。
...
- **#1 通信优化（保留协议）**：现有 poll+/events+去重回归测试打底；可选 clear-on-ack；Hub→OpenClaw 转发重试/死信 + 修 `os` 未导入 bug；事件附加字段 `level/title/data`（§6）。**不重写协议、不引 push**。
- **#2 后端服务最小闭环 + 干净生命周期**：FastAPI agent + Hub 后端（**先无 UI**）+ Bearer + 现有 poll 端到端 → 入库 → OpenClaw。agent/Hub 作为 **OS 后台常驻服务**运行（§7.1），优雅停机（停所有线程 + 杀托管子进程 + 释放端口），根治 V2 退出残留。控制 API（启停监控）走 loopback。
...
- **#5 UI / Tauri（共享前端，双角色）**：React + MUI（schema 驱动表单）+ Tauri 壳，连本地后端。**agent 控制台**（启停 Lada、设监听目录、ComfyUI 配置等——对齐并改进 V2 现有功能，不删）+ **Hub 仪表盘**（多机状态汇总）。每页先跑 ui-ux-pro-max `--design-system`。
...
- **协议/可靠性（优化项）**：现有 poll+去重回归不破坏；可选 clear-on-ack（Hub 取走后崩溃不丢）；Hub→OpenClaw 转发重试/死信；附加字段向后兼容（老消费者忽略）；鉴权 401 不清队列。
...
- **生命周期（修复 V2 残留）**：服务 stop/SIGTERM → 所有监控线程退出 + 托管子进程（lada-cli）被 terminate + 端口释放 + 进程真正结束（无残留）；Windows Service / launchd 安装-启动-停止-卸载冒烟。
...
| 风险 | 等级 | 缓解 |
|---|---|---|
| 在生产交易机跑额外 agent 影响交易进程 | 高 | agent 只读、低频、资源上限、最小产物、只 bind 私网；先在 distill 副本验证 |
...
| 事件投递优化引入回归 | 低 | 保留现有协议，优化均向后兼容；clear-on-ack 等为可选小步 |
...
1. **moomoo 四项体征实勘**（moomoo 在独立 Mac，已定为远程）：确认 (a) 进程管理器类型（本机已确认 pm2，他机可能 launchd）与作业名（`ecosystem.config.js`/launchd）；(b) `orchestrator_heartbeat.json` 路径与 grace；(c) OpenD 端口（默认 11111，可能被 `.env` 改）。（交易层信号不在范围。）
2. **通信优化范围确认**：保留 poll 协议；确认要做哪些可选优化（clear-on-ack / Hub 转发重试 / 附加字段）+ 私网绑定收紧（§3.2/§6）。
