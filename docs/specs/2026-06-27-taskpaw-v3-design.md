...
| # | 体征 | type_id | 配置 | 告警 |
|---|---|---|---|---|
| ① | **pm2 守护进程存活** | `process` | pm2 daemon（`pm2 ping` / 进程存在） | 不在 = 体系失去自愈根，所有进程不再自动重启 |
| ② | **orchestrator 进程在跑** | `process` | pm2 进程名（**可配**，默认 `orchestrator`，#0 确认 `ecosystem.config.js` 实际名）status | 非 online（errored/crash-loop/stopped） |
...
- 四项中 ①②④ 都用 `process`/`heartbeat` 这两种内置类型，③ 用 `tcp_check`；**moomoo preset 只需 process + heartbeat + tcp_check 三种插件**，无需 tail_jsonl/state_file 等。
- ④ 与 ② 互补：进程在跑 ≠ 在干活；心跳过期能抓到"进程活着但卡死"。
...
1. **moomoo 四项体征实勘**：确认交易机上 pm2 进程名（`ecosystem.config.js`）、`orchestrator_heartbeat.json` 路径与 grace、OpenD 端口（默认 11111，可能被 `.env` 改）、pm2 daemon 探测方式。（交易层信号已不在范围。）
