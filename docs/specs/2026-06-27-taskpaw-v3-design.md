...
7. **退出不干净（恼人）**：Windows app 点窗口 X 是 `withdraw()` 缩进托盘而非退出（`taskpaw.py:1880,2839-2848`），用户以为关了、进程仍在后台，要手动 kill。残留进程占着端口 5678 + 单实例 mutex（`taskpaw.py:2911-2916`）→ 重启实例拿不到端口、Hub 看到假 Stopped。**根因是"GUI app + 托盘"形态**，V3 用 headless 服务根治（见 §7.1）。
...
10. **干净生命周期（无托盘陷阱）**：agent 是 **headless 后台服务**，由 OS 服务管理器干净 start/stop；stop 必须终止所有监控线程 + 托管子进程（如 lada-cli）。**不再有"点 X 缩进托盘、进程残留"**（V2 痛点）。Hub 的 Tauri 壳退出时杀掉后端子进程（抄 MDCx）。见 §7.1。
...
在"保留 poll"前提下做安全优化：
- **agent 只 bind 私网网卡**（Tailscale/VPN 地址或局域网），**不** bind `0.0.0.0`；**只**开放 `/ping /status /events`，无管理 UI / 写接口。
...
├── agent/                   # 入口①: taskpaw-agent —— 最小化，默认无 UI，只 /ping /status /events
...
- 字段：`server_id`、`bind_addr`（私网网卡）、`api_token`（Bearer）、`monitors[]`。（poll 模型：agent 监听、Hub 来轮询，无需 hub_url。）
- `server_id`/机器名由 operator 指定并在 Hub 注册表登记（沿用 V2 做法）。
- 配置经 SSH/Ansible/运维脚本下发；agent **不**提供远程管理接口。

### 7.1 agent 运行形态与干净生命周期（修复 V2 退出残留痛点）

V2 的 GUI app + 托盘形态导致"点 X 缩进托盘、进程残留、要手动 kill"（§1.3#7）。V3 根治：

- **agent = headless 后台服务，无窗口、无托盘。** 由 OS 服务管理器托管：
  - Windows：注册为 **Windows Service**（pywin32）或 **NSSM/Task Scheduler** 包装 → `sc stop` / 服务管理器一键停，干净。
  - macOS：**launchd** plist（与 moomoo 自身 launchd/MacSubs 现状一致）。
- **优雅停机**：收到服务 stop / SIGTERM → supervisor 依次 `stop()` 所有 MonitorInstance（终止轮询线程）+ **terminate 托管子进程（如 lada-cli managed 模式）** + 释放端口 → 退出。无残留、无僵尸子进程。
- **单实例**：保留命名 mutex/端口检查，但因停机干净，不再有"残留占端口→新实例拿不到→Hub 假 Stopped"（§1.3#7）。
- **看状态去哪**：不在每台机装 GUI；**统一看 Hub 的 Web UI**（所有 agent 状态汇总）。需要本机自检时，agent 可选暴露一个**只读**本地状态页（非管理界面）。
- **Hub 退出**：Hub 有 Tauri 壳 → 壳退出时**杀掉它 spawn 的后端子进程**（抄 MDCx 的 child-process 清理），同样不留后台。
...
- **#2 headless 最小闭环 + 干净生命周期**：FastAPI agent + Hub（**先无 Tauri、无花哨前端**）+ Bearer + 现有 poll 端到端 → 入库 → OpenClaw。agent 作为 **OS 后台服务**运行（§7.1），实现优雅停机（停所有线程 + 杀托管子进程 + 释放端口），根治 V2 退出残留。
- **#2.5 安全验收**（评审 Kimi P2#10 —— 独立 issue 防遗漏）：覆盖 §3.1/§3.2 安全项（agent 只 bind 私网、token 不进日志、Hub UI 的 CORS/origin、agent 无管理 UI、secret 不入 `/status`、Bearer 401 不清队列）。
...
- **安全（评审 P3#14）**：HTTP/WS 鉴权、Hub UI 的 CORS/origin 校验、token 不进日志、agent 只 bind 私网且无管理 UI、secret 不入 `/status`、Bearer 401 不清队列。
