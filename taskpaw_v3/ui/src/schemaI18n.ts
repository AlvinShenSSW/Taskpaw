...
const BY_TYPE: Record<string, Record<string, FieldT>> = {
  ...
  jasna: {
    ...
    av_translate: {
      title: "AV 翻译",
      description:
        "默认关。开：每个文件修复完成后，用 WhisperJAV 识别日语语音，再用「设置」里的 LLM API 翻译成简体中文，在修复后的视频旁边生成与视频同名的中文字幕 <原名>-破解.srt（3.5 及更早版本修复的 <原名>_restored.mp4 则生成 <原名>_restored.srt）。日文字幕 <原名>-破解.ja.srt 只是中间产物：中文字幕写好后自动删除；翻译没完成（失败、跳过、缺 API key、停止）时保留，下次启动只补翻译。输出文件夹里已修复但还没有字幕的影片，会在待修复文件全部处理完之后补做。会在输出文件夹里创建 .avsubs/ 工作文件夹。需要先在「设置」里配置 LLM API。已有字幕的影片不再生成字幕：修复后的视频旁边已有 <原名>-破解.srt（或 .ass / .ssa / .vtt），或者有标记不是日文的 <原名>-破解.<标记>.srt（例如 <原名>-破解.chs.srt；日文标记为 ja、jp、jpn、jap、japanese、日语、日文、日本語，也包括 ja-JP 这类写法）；输出文件夹里其他影片的字幕不算。输出文件夹读不到时跳过字幕（提醒一次，下次启动再试），已有的字幕文件绝不覆盖。已有的 .ja.srt / .srt 会原样复用：如果同名替换了源视频，请先删掉旧的 .srt 文件。",
    },
    whisperjav_exe_path: {
  ...
  avsubs: {
    avsubs_root_folder: {
      title: "片库文件夹",
      description:
        "要扫描的视频片库文件夹（必填）。每个还没有字幕的视频，会在旁边生成 <原名>.srt（简体中文）。满足任意一条就算已有字幕、整部跳过：(a) 有 <原名>.srt（或 .ass / .ssa / .vtt）；(b) 有标记不是日文的 <原名>.<标记>.srt，例如 <原名>.chs.srt、<原名>.zh.srt（日文标记为 ja、jp、jpn、jap、japanese、日语、日文、日本語，也包括 ja-JP 这类写法）；(c) 所在文件夹里只有这一部视频，并且有任何不是日文的字幕文件（名字不限）。读不到的文件夹本次跳过（提醒一次，下次启动再试），已有的字幕文件绝不覆盖。日文字幕 <原名>.ja.srt 只是中间产物：中文字幕写好后自动删除，翻译没完成时保留；已存在的 .ja.srt 会直接复用（只做翻译），完成后同样删除。隐藏、链接或挂载的文件夹，以及 macOS 的 ._ 元数据文件，会被跳过。会在该文件夹下创建 .avsubs/ 工作文件夹。与 Jasna 共用 GPU：两者同时运行时按文件轮流使用，等待的一方显示「waiting for GPU (held by …)」。需要先在「设置」里配置 LLM API。不会随 TaskPaw 开机自启，需手动点「启动」。注意：不要让两个运行中的任务覆盖相互重叠的文件夹，也不要指向已开启「AV 翻译」的 Jasna 任务的输出文件夹。",
    },
    avsubs_recursive: {
      title: "扫描子文件夹",
