...
// SVG icon shapes per monitor type / preset id (#93), traced from the design
...
// Rendered as JSX (not dangerouslySetInnerHTML) so the markup can never carry
...
const SHAPES: Record<string, ReactNode> = {
  ...
  comfyui: (
    <>
      ...
      <path d="M6 9v2a3 3 0 0 0 3 3M18 9v2a3 3 0 0 1-3 3" />
    </>
  ),
  ...
  tcp_check: (
    <>
      <rect x="3" y="4" width="18" height="5" rx="1" /><rect x="3" y="15" width="18" height="5" rx="1" />
...
export function ServiceIcon({ id, size = 21 }: { id: string; size?: number }) {
  const shape = SHAPES[id] ?? SHAPES._fallback;
  return (
    <Box
      aria-hidden="true"
      sx={{
