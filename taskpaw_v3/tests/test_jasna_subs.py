...
def test_tasklog_bulk_missing_exe(tmp_path, monkeypatch):
    h = _setup(tmp_path, monkeypatch, restored=("a.mp4", "b.mp4"), exe=False)
    h.inst.start(h.emit)
...
def test_tasklog_asr_finishes_during_cancel_not_interrupted(tmp_path, monkeypatch):
    h = _setup(tmp_path, monkeypatch, restored=("LMNO-123.mp4",))
    h.inst.start(h.emit)
...
def test_tasklog_stop_asr_exited_or_live(tmp_path, monkeypatch, rc):
    h = _setup(tmp_path, monkeypatch, restored=("a.mp4", "b.mp4", "c.mp4", "d.mp4"))
    h.inst.start(h.emit)
    if rc is not None:
...
def test_tasklog_launch_errors_do_not_copy_argv(tmp_path, monkeypatch):
    h = _setup(
        tmp_path,
        monkeypatch,
        restored=("a.mp4", "b.mp4", "c.mp4", "d.mp4"),
...
def test_tasklog_settle_only_one_skip(tmp_path, monkeypatch):
    h = _setup(tmp_path, monkeypatch, restored=("a.mp4",), key=False)
    h.inst.start(h.emit)
    h.spawner.last.finish()
    h.inst.check(h.emit)
    h.inst.check(h.emit)
