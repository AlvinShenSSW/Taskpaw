...
from taskpaw_v3.monitors.plugins.host_metrics import (
    HostMetricsConfig,
    HostMetricsPlugin,
)
...
def fake_psutil(
    cpu=10.0, mem=20.0, disk=30.0, sent=0, recv=0, mem_total=16 * 1024**3, mem_used=None
):
    ...
    return ns(
        ...
        virtual_memory=lambda: ns(
            percent=mem, used=used, total=mem_total, available=mem_total - used
        ),
...
def test_metrics_keys_and_gpu_na(monkeypatch):
    ...
    assert m["mem_total_mb"] == 16 * 1024 and m["mem_used_mb"] == round(
        16 * 1024 * 0.34
    )
...
def test_mem_and_disk_immediate_alert(monkeypatch):
    ...
    assert (
        st.state == "degraded"
    )  # a valid MonitorStatus state (not the event level "warn")
...
def test_read_gpu_windows_parses_util_and_vram(monkeypatch):
    ...
    def fake_run(*a, **k):
        return _t.SimpleNamespace(
            returncode=0, stdout="50, 2048, 8192\n70, 1024, 8192\n"
        )
...
def test_agent_host_metrics_can_be_disabled():
    ...
    mons = effective_monitors(
        AgentConfig(server_id="s", machine="dev", host_metrics=False)
    )
