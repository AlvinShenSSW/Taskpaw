...
def fake_psutil(cpu=10.0, mem=20.0, disk=30.0, sent=0, recv=0,
                mem_used=8192 * 1048576, mem_total=16384 * 1048576):  # 8 GB / 16 GB
    ns = types.SimpleNamespace
    return ns(
        cpu_percent=lambda interval=None: cpu,
        virtual_memory=lambda: ns(percent=mem, used=mem_used, total=mem_total),
...
def test_metrics_keys_and_gpu_na(monkeypatch):
    ...
    m = st.metrics
    assert set(m) == {"cpu_pct", "mem_pct", "mem_used_mb", "mem_total_mb",
                      "disk_pct", "net_in_bps", "net_out_bps",
                      "gpu_pct", "gpu_mem_used_mb", "gpu_mem_total_mb"}
    assert m["cpu_pct"] == 12.0 and m["mem_pct"] == 34.0 and m["disk_pct"] == 56.0
    # System RAM reported as MB amounts too (#128): 8 GB used / 16 GB total.
    assert m["mem_used_mb"] == 8192 and m["mem_total_mb"] == 16384
