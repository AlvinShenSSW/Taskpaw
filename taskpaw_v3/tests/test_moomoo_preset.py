...
def test_preset_overrides_apply():
    specs = {
        s["config"]["name"]: s["config"]
        for s in moomoo_preset(
            opend_port=22222, heartbeat_path="/tmp/hb.json", grace_seconds=900
        )
    }
...
def test_build_supervisor_registers_all_monitors():
    ...
    sup = build_supervisor(
        reg, moomoo_preset(heartbeat_path="/tmp/x.json"), q, "moomoo"
    )
...
def test_build_supervisor_rejects_unknown_type():
    ...
    with pytest.raises(ValueError):
        build_supervisor(
            default_registry(), [{"type_id": "nope", "config": {"name": "x"}}], q, "m"
        )
...
def test_queue_sink_adapter_enqueues_with_level_and_title():
    ...
    sink(
        ...
        "alert",
        "moomoo-opend down",
        "127.0.0.1:11111 not accepting connections",
    )
...
def test_queue_sink_drops_unknown_level_gracefully():
    ...
    make_queue_sink(q, "m")(
        "inst", "weird", "t", "msg"
    )  # invalid level → omitted, not crash
...
def test_failed_enqueue_not_marked_delivered_so_keyed_alert_retries():
    ...
    sup._emit(
        "f", "alert", "f degraded", "msg", None, "f:degraded"
    )  # add fails → not recorded
    assert "f:degraded" not in sup._monitors["f"].seen_dedupe
    sup._emit(
        "f", "alert", "f degraded", "msg", None, "f:degraded"
    )  # retries, succeeds now
