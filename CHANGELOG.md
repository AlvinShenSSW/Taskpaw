...
- **新的「本轮影片」卡片**：Jasna 任务（开启 AV 翻译时）的任务页中下部，列出这次点「开始」以来的每部片：
  片名、修复结果（完成 / 失败 / 本轮前已修复）、翻译结果（翻译完成 / 部分保留日文 N 句 / 未翻译·无语音 / 已有字幕 /
  未开始（修复失败）/ 识别失败 / 翻译失败 / 跳过及原因）、**用了哪些模型、各翻了几句**（例如 grok-4.3 · 540 句、
  deepseek-chat · 12 句）、用时和完成时间。
- **筛选**：默认「已完成」（最新完成的在前），也可以选「未完成」（当前那部在最前，其余按排队顺序）或「全部」；
  每个选项都显示数量。上方有本轮合计：翻译完成 / 部分保留日文 / 已有字幕 / 未翻译 / 失败 / 修复失败，加起来等于已完成数。
- **每页 10 部**，上一页 / 下一页；手机宽度下自动改成一行一项的竖排，不会横向滚动。
- 这张卡片替代 Jasna 任务原来的「影片」分页列表；AV 翻译任务、Hub 页面和手机推送不变。「本轮」指这次开始以来，
  重启后从头算，更早的记录在「日志」页。

---

## V3 3.9.2 — 翻译提速：DeepSeek / MiMo 关闭思考，失败原因写进日志，批次能恢复（#201）

- **关闭思考**：设置里每个模型（主模型 / 备用模型 1 / 备用模型 2）新增「思考」选项：自动 / 关闭思考 / 不发送（服务默认）。
  「自动」时，DeepSeek（api.deepseek.com）和 MiMo（xiaomimimo.com）会**自动关闭思考**，翻译更快更稳，其它服务不发这个参数。
  **升级后不用改设置**：已经配好的 DeepSeek / MiMo 会自动生效。实测 mimo-v2.6-flash 关闭思考后快 2–3 倍，deepseek-flash
  预计也会少很多「回复被截断」（新日志会写明每次失败的原因）。已验证：deepseek-flash、mimo-v2.6-flash；同一服务的其它模型如果不接受这个参数，
  TaskPaw 会自动改为不发送并继续翻译。也可以手动选「不发送（服务默认）」。「测试」按钮按同样的设置发请求。
- **失败原因写进日志**：翻译请求失败时，日志里会写明原因（例如回复被截断、空回复、格式不对），不再只写「无效回复」。
- **批次能恢复**：以前一次超时或截断后，每批的句数会减半（最少 5 句）并且整轮都不再变大；现在连续几批顺利后会逐步
  恢复到 40 句一批。已经证明太大的批次不会反复再试。
- **被截断的一句不再算作"被拒绝"**：单句因为额度不够被截断时，以后还会再问（也会问备用模型），不会被记成该模型拒绝。
  注意：3.9.1 时已经记下的"拒绝"会保留在该片的续翻进度里；想让这部片全部重新翻译，删掉它的续翻进度即可。
- 同一个模型、只差这个选项的两个模型槽会被当作同一个模型（只保留第一个）。

---

## V3 3.9.1 — AV 翻译「影片」列表：所有影片都能翻页看到，每页 10 部（#198）

- **完整片单，分页浏览**：AV 翻译任务的「影片」列表不再只显示 12 部、其余折叠成「还有 N 部」。在 TaskPaw 的
  任务详情里，列表按顺序列出本次任务的每一部片，每页 10 部，用「上一页 / 下一页」翻页，显示「第 x / y 页 · 共 N 部」。
- **包括历史**：开始前就已经有字幕的片（例如之前几次运行已经翻译好的片）也会列在后面，标为「已有字幕」；同名冲突
  没有处理的片标为「同名冲突，未处理」。整个片库都已翻译完时，也能翻看这份列表。
- **跟着当前影片走**：默认打开正在处理的那一部所在的页，并随处理进度自动翻页；手动翻页后停在你看的那一页，
  点「回到当前影片」即可回去。列表每 5 秒刷新一次状态。
- Hub 页面、手机/OpenClaw 推送的内容不变（仍是精简版的前 12 行）；Jasna 任务的列表同样可以翻页。

---

## V3 3.9.0 — 日志：每个任务、每个进程做了什么，一目了然（#196）

- **新的「日志」页**（代替原来的「事件」页）：按时间倒序列出每个任务做了什么，用中文（或英文界面下的英文）一句话写清楚：
  哪个任务、哪部片、哪个进程（名字 + PID），比如「Lada 开始转码 xx（第 3/12 部）」「完成转码 → xx-破解.mp4，用时 57 分」
  「转码失败：退出码 1」「开始翻译：620 句，模型 grok-4.3」「grok-4.3 暂时不可用，改用 deepseek-chat」「grok 拒绝了 1 句」
  「翻译完成：grok 619 句 / deepseek 1 句」。点开一条能看到退出码、子进程最后几行输出、模型、句数等细节。
- **覆盖**：TaskPaw 启动/关闭（上次若是直接关掉窗口，下次启动会注明「上次非正常退出」并补记当时被中断的片子）；你的
  启动 / 停止 / 添加 / 删除 / 修改任务；Lada / Jasna 的修复开始、完成、失败、重试、跳过；等待 GPU / 获得 GPU；WhisperJAV
  识别；翻译的开始、模型切换（不可用 / 恢复 / 设置改了）、被拒句数、暂缓 / 暂停 / 续翻、完成；字幕写入与跳过；停止时
  被中断的工作；以及原有的所有提醒（以小字「提醒」行显示）。拆句、重试会合并成一条，不会刷屏。
- **筛选与导出**：按日期（今天 / 昨天 / 更早）、任务、严重程度（信息 / 警告 / 错误）筛选，按片名或模型搜索，实时刷新，
  可「加载更早」，可把当天筛选结果导出成 txt。每个任务的面板里有「最近日志」（最近 8 条，跨天）。
- **保存**：只在本机，放在 TaskPaw 数据目录的 `logs\` 里，每天一个文件，保留 30 天（总量超过 50 MB 时先删最早的，
  最近 7 天一定保留）。不推送到手机 / OpenClaw，原有的提醒照旧。日志里不记 key、命令行参数或字幕内容。

---

## V3 3.8.0 — AV 翻译：断点续翻 + 两个备用模型，被拒/断线/停止都不再丢整部片（#192 #190）

- **断点续翻**：每部片的翻译按句写进进度文件（放在 TaskPaw 自己的数据目录 `subs-checkpoints`，不写进片库），
  每翻完一批就原子写盘。停止、崩溃、断网、重启之后，下次只翻还没翻的句子；中文 `.srt` 写好后才删进度文件。
  日文字幕变了就对不上号，自动从头翻。
- **两个备用模型**（设置 → 备用模型 1 / 备用模型 2：API 地址、模型、key，各自保存 / 清除 / 测试；key 和主模型
  一样只显示 `***`、不进日志；环境变量 `TASKPAW_LLM_FALLBACK1_API_KEY` / `…2…` 可覆盖）。「测试」现在发的是
  真正的翻译试探请求，保存时就能看出该模型接不接受我们的翻译提示。
- **被拒只影响那几句**：主模型拒绝（HTTP 401/402/403/404、内容过滤、空回答等）时先发一个无害的试探请求：
  试探成功 → 是内容被拒 → 把这批拆到单句，只有被拒的那几句交给备用模型 1、再备用模型 2；三个都拒就保留日文，
  整部片照常出中文字幕。试探失败 → 是 key/额度/服务问题 → 该模型暂停（熔断），一次提醒，自动切到备用模型。
- **临时错误先等再试**：超时、断网、5xx、429 等按 10 秒 / 30 秒 / 90 秒重试（服务方给了 `Retry-After` 就按它，
  最多 5 分钟）；还不行再试探，不会无限重试。
- **熔断与故障切换**：模型暂停 5 → 15 → 30 分钟，到点试探通过就恢复；期间自动改用下一个模型
  （设置里「主模型不可用时改用备用模型」默认开）。
- **不堵队列、不判失败**：某部片只差等某个暂停中的模型时先放一边，后面的片子照常翻；累计等了 2 小时就记为
  「翻译服务不可用，可续」（跳过，不算失败），下次 Start 接着翻，并提醒一次。设置里模型全部清空时，这些片子
  记为「缺 LLM key」跳过，进度保留，绝不整部以日文发布。
- **进度显示**：翻译步骤显示正在用的模型，以及「已续翻 / 备用模型 / 保留日文」句数和「等待翻译服务」。
  运行结束的汇总里注明保留日文的句数和暂停的片数。
- Jasna 与独立任务对「缺 LLM key」的处理统一为跳过（不再算失败）。

---

## V3 3.7.1 — AV 翻译：识别已有字幕（.chs / .zh / 其他名字），扫描读不到就跳过，绝不覆盖已有字幕（#191）

- **已有字幕的片子整部跳过**（独立「AV 翻译」任务与 Jasna「AV 翻译」）。满足任意一条就算已有字幕：
  - (a) `<片名>.srt`（以及 `<片名>.ass` / `.ssa` / `.vtt`）；
  - (b) `<片名>.任意标记.srt` / `.ass`，例如 `<片名>.chs.srt`、`<片名>.zh.srt`，但日文标记的除外
    （`ja`、`jp`、`jpn`、`jap`、`japanese`、`日语`、`日文`、`日本語`，含 `ja-JP` 这类写法）；
  - (c) 仅独立任务：片子所在文件夹里只有这一部视频，并且有任何不是日文的字幕文件（名字不限，例如
    `LMNO-047-破解-4K-C.zh.srt`、`dl.example.com@pqrs00948.srt`）。Jasna 的输出文件夹放着所有修复好的片子，
    这一条对 Jasna 不适用。
  - 同一个文件夹里有 cd1…cdN 多段时，每段只认自己名字开头的字幕；`Movie.part2.srt` 只属于
    `Movie.part2.mp4`，不会算到 `Movie.mp4` 头上。
- **扫描读不到就跳过**：字幕状态改为从文件夹列表判断，不再逐个文件探测；读不到的文件夹照旧报错并跳过。
  Jasna 的输出文件夹读不到（例如 NAS 掉线）时本次不做字幕并提醒；文件夹确实不存在时照常。
- **开工前再确认一次**：WhisperJAV 开始识别前、以及提交翻译前，重新看一眼片子所在文件夹；这期间放进了
  字幕就直接跳过（已有字幕），读不到就跳过这部片（下次 Start 再试），并且每次运行只提醒一次。
- **绝不覆盖**：写 `.ja.srt` / `.srt` 时，目标位置已有文件就不写、不替换，这部片记为跳过（已有字幕），
  本次运行自己生成的 `.ja.srt` 顺手删掉，片库里原有的保留。
- 字段说明（英文 + 中文界面）、README、OpenClaw 集成文档同步更新。

---

## V3 3.7.0 — AV 翻译进度重做：每部片 修复 → 识别 → 翻译 分步进度，翻译时显示模型，队列只在字幕完成后算完成（#189）

- **分步进度**（Jasna 开启「AV 翻译」时，以及独立「AV 翻译」任务）：代理控制台不再在修复结束后只显示
  `1/1 已完成`。每部片显示自己的步骤（Jasna：修复 → 识别 → 翻译；独立任务：识别 → 翻译），当前步骤有
  自己的进度条和指标：修复沿用帧进度 / 速度 / 预计剩余；识别显示 WhisperJAV 场景 `i/N`（或阶段 `k/8`）、
  已用时间和预计剩余；翻译显示批次、句数、预计剩余，以及正在使用的模型（如 `grok-4.3 · api.x.ai`，只含
  主机名，不含 key、账号或端口）。等待 GPU 时显示被谁占用、已等多久。
- **批量队列**：同时处于不同阶段的影片（一部在翻译、下一部在修复）逐行列出，最多 12 行，其余显示
  「还有 N 部」。队列卡片区分「完成 / 翻译中 / 失败 / 跳过 / 排队 / 已有字幕」（独立任务）或「修复 a/b ·
  字幕」（Jasna）。
- **WhisperJAV 进度解析**：从识别子进程最近 40 行输出里读取 qwen / decoupled 流水线的阶段与场景行
  （WhisperJAV 1.9.3 实测格式），第 5 阶段的标志行被刷出尾部时也能从后续行推断，不会卡在 8%；其他引擎只
  显示已用时间。进度只增不减，字幕完成前最多 99%。
- **语义变化（仅 Jasna 开启 AV 翻译时）**：`queue_completed` 改为「修复完成且字幕已结算（成功、失败或
  跳过）」的影片数，`queue_remaining = 总数 − 完成 − 失败`，状态详情里的 `X/Y done` 同步；新增
  `queue_restored`（已修复数）。中止详情和 `done` 事件文字不变。独立任务新增 `queue_pre_done`（原本已有
  字幕）。关闭 AV 翻译的 Jasna、Lada 和其他任务的指标与界面完全不变。
- **Hub status.md**：Jasna / 独立任务一行在原有 `X/Y done (Z left) | 文件 |` 之后追加当前阶段，例如
  `识别 43% · 约剩 6 分`、`翻译 56% · 约剩 1 分 · grok-4.3 · api.x.ai`、`等待 GPU（AV）`，Jasna 再追加
  `修复 a/b · subs S/T`。LADA 行逐字节不变。
- 新增指标 `film`、`steps`、`films`、`films_more`、`model`、`queue_restored`、`queue_pre_done`，写入
  OpenClaw 集成文档（含行与计数的对应关系）。
- 纯观察：处理顺序、GPU 租约、字幕结算与发布都没有改动。

---

## V3 3.6.0 — Jasna 输出改名 `<片名>-破解.mp4` / `.srt`，中文字幕写好后删除 `.ja.srt`（#187）

- **Jasna 输出命名**：修复后的视频改为 `<片名>-破解.mp4`（例：`SDAB-312-破解.mp4`），暂存文件
  `<片名>-破解.tmp.mp4`（退出码 0 才 `os.replace` 成正式名；hev1→hvc1 改标签不变）。
- **兼容旧名**：已存在的 `<片名>_restored.mp4` 仍算已修复，不会重跑、也不会改名；新旧两个都在时以新名
  为准。同名冲突检测不变（两个源文件主名相同 → 后一个跳过）。
- **字幕跟着它旁边的视频命名**（播放器只自动加载与视频同名的字幕）：新修复的影片得到
  `<片名>-破解.srt`；需要补字幕的旧 `<片名>_restored.mp4` 得到 `<片名>_restored.srt`，WhisperJAV
  识别的也正是这个文件。
- **中文字幕写好后删除 `.ja.srt`**（Jasna「AV 翻译」与独立「AV 翻译」任务都一样）：翻译成功、无人声
  （空字幕）、以及 0 句的「只补翻译」三种完成情况，都在中文 `.srt` 发布成功、任务记为完成后删掉对应的
  `.ja.srt`；独立任务里片库原有的 `.ja.srt` 也在中文字幕生成后删除。其余情况（翻译失败、跳过、缺
  LLM key、取消、停止时只写出了 `.ja.srt`）一律保留，作为下次启动「只补翻译」的断点，续跑成功后再
  删除。删除失败只记一条警告，不影响任务结果。新增 `SubsJob.discard_ja()`。
- **启动清扫**：孤立暂存文件的清扫同时覆盖 `*-破解.tmp.mp4` 与旧的 `*_restored.tmp.mp4`（保留/年龄
  规则不变）；某个源文件重新开跑时顺带清掉它旧名的残留暂存。字幕发布临时文件的清扫同时覆盖
  `*-破解*.srt.*.tmp` 与 `*_restored*.srt.*.tmp`，仍有 10 分钟年龄门槛。
- 字段说明（英文 + 中文界面）、README、OpenClaw 集成文档同步更新。
